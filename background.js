...
async function updateAlarm() {
  ...
  if (chrome.storage && chrome.storage.managed) {
    try {
      ...
      // Managed storage not supported or empty
    }
  }
...
async function getUsername() {
  ...
  if (chrome.storage && chrome.storage.managed) {
    try {
      ...
      if (managed && managed.defaultRequestingUser !== undefined) {
        const val = managed.defaultRequestingUser;
        if (typeof val === 'string') {
          ...
          console.error('Bad configuration value: Enterprise policy "defaultRequestingUser" must be a string, got:', val);
        }
      }
    } catch (e) {
...
function isValidUrl(urlStr) {
  const formatted = validateAndFormatUrl(urlStr);
  if (!formatted) return false;
  try {
    const testUrl = toHttpScheme(formatted);
...
async function syncPrinters(onProgress, isInteractive = false) {
  ...
  activeSyncPromise = (async () => {
    ...
    let rawCupsServers = [];
    let rawIppPrinters = [];
    if (managedItems.cupsServers !== undefined || managedItems.ippPrinters !== undefined) {
      console.log('Using enterprise policy printer/server configuration.');
      rawCupsServers = managedItems.cupsServers || [];
      rawIppPrinters = managedItems.ippPrinters || [];
    } else {
      console.log('Using user sync printer/server configuration.');
      rawCupsServers = syncItems.cupsServers || [];
      rawIppPrinters = syncItems.ippPrinters || [];
    }

    // Validate and filter CUPS servers, complain for bad values
    const cupsServers = [];
    for (const url of rawCupsServers) {
      if (isValidUrl(url)) {
        cupsServers.push(validateAndFormatUrl(url));
      } else {
        console.error(`Bad configuration value: CUPS server URL "${url}" is invalid.`);
      }
    }

    // Validate and filter standalone IPP printers, complain for bad values
    const ippPrintersMap = new Map();
    for (const item of rawIppPrinters) {
      const norm = normalizeIppPrinter(item);
      if (norm && isValidUrl(norm.url)) {
        norm.url = validateAndFormatUrl(norm.url);
        if (!ippPrintersMap.has(norm.url)) {
          ippPrintersMap.set(norm.url, norm);
        }
      } else {
        console.error(`Bad configuration value: IPP printer URL is invalid or missing:`, item);
      }
    }
    const ippPrinters = Array.from(ippPrintersMap.values());

    // Complain in the logs if no options are available
    if (cupsServers.length === 0 && ippPrinters.length === 0) {
      console.error('No printers or CUPS servers configured.');
    }
    ...
    const serverTask = async (serverUrl) => {
      ...
      try {
        ...
        console.error(`Server/Printer is unreachable: CUPS server at "${serverUrl}" is unreachable. Error: ${e.message}`);
    ...
    const printerTask = async (printer) => {
      ...
      try {
        ...
        if (!successUrl) {
          ...
          console.error(`Server/Printer is unreachable: Standalone printer "${printer.name || originalUrl}" is unreachable. Error: ${msg}`);
